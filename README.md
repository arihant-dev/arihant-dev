# Hi there, I'm Arihant Jain 👋

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/ariihanjain)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/arihant-dev)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:jainarj411@gmail.com)

## 🚀 About Me

I'm a **Software Development Engineer** passionate about building scalable distributed systems and elegant solutions. Currently pursuing my **MSc in Computer Science at EPITA, Paris**, while bringing 3+ years of professional experience in backend development, microservices architecture, and iOS development.

🌍 Based in **Cachan, France**  
🎓 Previously studied at **Jaypee Institute of Information Technology, India**  
💼 Former **SDE II at Thriwe**, where I architected high-scale distributed systems

## 💻 Technical Stack

### Languages
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat-square&logo=go&logoColor=white)
![Swift](https://img.shields.io/badge/Swift-FA7343?style=flat-square&logo=swift&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat-square&logo=dart&logoColor=white)

### Frameworks & Tools
![Kafka](https://img.shields.io/badge/Apache_Kafka-231F20?style=flat-square&logo=apache-kafka&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat-square&logo=flutter&logoColor=white)
![SwiftUI](https://img.shields.io/badge/SwiftUI-000000?style=flat-square&logo=swift&logoColor=white)

### Specializations
- 🏗️ **Distributed Systems** & Microservices Architecture
- ⚡ **Event-Driven Architecture** with Kafka
- 🔄 **CI/CD** with GitHub Actions & Jenkins
- 📱 **iOS Development** (UIKit, SwiftUI)
- 🎯 **REST APIs & gRPC**

## 🏆 Professional Highlights

- 🔧 **Architected** a centralized communications service handling SMS, email, and WhatsApp at scale
- 🚀 **Led migration** from monolithic PHP to distributed Golang microservices with Kafka integration
- ⚙️ **Automated CI/CD pipelines** reducing release cycles by 40%
- ⛳ **Designed** real-time golf booking system improving efficiency by 60%
- 🏦 **Contributed** to Benefits-as-a-Service platform adopted by FAB, HSBC, and Mastercard
- 🔬 **Researched** terahertz sensors with meta-material absorbers at DRDO

## 🌱 Current Focus

- 📚 Deepening knowledge in advanced algorithms and distributed systems at EPITA
- 🔭 Exploring cloud-native architectures and observability patterns
- 🤝 Open to collaborating on interesting open-source projects
- 💡 Interested in systems programming, backend infrastructure, and developer tools

## 📊 GitHub Stats

![Arihant's GitHub stats](https://github-readme-stats.vercel.app/api?username=arihant-dev&show_icons=true&theme=radical)

## 🎯 Interests

- 🏗️ Building scalable backend systems
- 📱 Mobile app development (iOS)
- ☁️ Cloud infrastructure & DevOps
- 🎮 System design & architecture patterns
- 📖 Continuous learning & mentoring

## 📫 Let's Connect!

Feel free to reach out for collaborations, interesting projects, or just a chat about technology!

- 📧 Email: jainarj411@gmail.com
- 💼 LinkedIn: [ariihanjain](https://linkedin.com/in/ariihanjain)
- 🌐 Location: Paris, France

---

⭐️ From [arihant-dev](https://github.com/arihant-dev)